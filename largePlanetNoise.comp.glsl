...
vec3 fbm(vec3 vertex) {
    float scaleFactor = min(sqrt(1 / (objScale)), 0.1f);
    float scaleFactor2 = 0.2f;
    float scaleFactor3 = 0.1f;
    ...
    vec3 noiseMask = snoise((vertex + offset2 + 2.5f) / 2.0f) * vertex;

    return (noise0 + noise1 + noise2) * abs(noiseMask) + noise3 + noise4 + noise5;
}
