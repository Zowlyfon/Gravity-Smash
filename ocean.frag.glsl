...
vec3 fbm(vec3 pos, vec3 color) {
    vec3 noise0 = snoise((pos + offset) * 4.0f) * color;
    vec3 noise1 = snoise((pos + offset) * 8.0f) * color * 0.5f;
    vec3 noise2 = snoise((pos + offset) * 16.0f) * color * 0.25f;
    vec3 noise3 = snoise((pos + offset) * 32.0f) * color * 0.125f;
    vec3 noise4 = snoise((pos + offset) * 64.0f) * color * (1.0f/16.0f);
...
void main()
{
    ...
    vec3 fragColor = modelColor + fbm(aPos, modelColor) * 0.5;
    ...
    float gamma = 2.2f;
    FragColor = vec4(pow(result, vec3(1.0f/gamma)), 0.75f);
}
